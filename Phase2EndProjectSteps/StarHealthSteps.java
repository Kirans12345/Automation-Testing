...
public class StarHealthSteps {
	...
	WorkingWithHomePage homePage = new WorkingWithHomePage(BackgroundClass.driver); 
	WorkingWithBuyNow buyNowPage = new WorkingWithBuyNow(BackgroundClass.driver);

	@Given("User launches the Star Health application with{string}")
	public void
	user_launches_the_star_health_application_with(String URL) {

		BackgroundClass.driver.manage().window().maximize(); BackgroundClass.driver.get(URL);
	...
	InterruptedException {
		Thread.sleep(2000); homePage.click_On_BuyNow();
	}
	...
	InterruptedException {
		ph_no = PhNo; buyNowPage.give_phno(PhNo);
	}
	...
	public void user_clicks_on_i_need_health_insurance_from_the_drop_down_menu () throws InterruptedException {
		buyNowPage.click_dropdown(); Thread.sleep(1000);
	}

	@When("User selects the option {string}")
	public void user_selects_the_option(String OptionPlan)
			throws InterruptedException {
		BackgroundClass.driver.findElement(By.xpath("//li[text()="
				+ "'" + OptionPlan + "'" + "]")).click(); Thread.sleep(3000);
	}
	...
	public void user_validates_that_the_mobile_number_is_the_same_as_the_previously_entered_number_using_a_j_unit_assertion()
			...
			String phoneNumber = buyNowPage.ContactNo_verify(); Assert.assertEquals("Phone Number Matched ", ph_no,
					phoneNumber);
	}
